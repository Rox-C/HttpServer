...
namespace http
{
    ...
    void HttpRequest::setQueryParameters(const char *start, const char *end)
    {
        ...
        // 处理最后一个参数
        std::string lastPtr = argumentStr.substr(prev);
        std::string::size_type equalPos = lastPtr.find('=');
        if(equalPos != std::string::npos)
        {
            std::string key = lastPtr.substr(0,equalPos);
            std::string value = lastPtr.substr(equalPos + 1);
            queryParameters_[key] = value;
        }
    }
    ...
    void HttpRequest::addHeader(const char *start, const char *colon, const char* end)
    {
        ...
        while(!value.empty() && isspace(value[value.size() - 1])) // 去掉结尾的空格
        {
